		...
		if ( $query->have_posts() ):        	       
			...
			while ( $query->have_posts() ) :$query->the_post();
					...
					echo '<div class="item">';
						...
						$cat_display = ot_get_option( 'large_category_display', 'vertical');

						if( $cat_display != 'vertical' ){
							$catname = $cat;
						}else{
						...
						echo ($category_display == 'on')? '<a class="slide-cat category-'.$category[0]->term_id.' '.$cat_display.'" href="'.get_category_link($category[0]->term_id).'">'.$catename.'</a>' : '';
						}
						echo '<div class="slideinfo scaleUp">';
							...
							echo ($excerpt_display == 'on')? '<p>'.wp_trim_words( get_the_excerpt(), $num_words = $excerpt_length, '' ).'</p>' : '';
						echo '</div>';						
