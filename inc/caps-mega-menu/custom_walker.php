            ...
            if(in_array('cat-posts', $item->classes)){
				...
				foreach ( $myposts as $post ) : setup_postdata( $post );
						...
						if($item->displaypost == 'small'){
							...
							<span class="date-meta"><i class="genericon genericon-time"></i>'.get_the_date( 'M d, Y', $post->ID ).'</span></div></div>';
							...
							$thumbshow .= '<span class="date-meta"><i class="genericon genericon-time"></i>'.get_the_date( 'M d, Y', $post->ID ).'</span>';
						   	$thumbshow .= '</div>';
						   	$thumbshow .= '</div>';
						}
						$thumbshow .= '</li>';
	            ...
	            if(($item->thumb == 1) && ($item->type == 'post_type')){
						  ...
						  if ( has_post_thumbnail($item->object_id)) {
						   $thumbshow .= '<div class="image"><a href="'.$item->url.'"><img src="'.$thumb.'" alt="'.$item->title.'"></a></div>';
							...
							$thumbshow .= ( get_post_type( $item->object_id ) == 'post' )? '<span class="date-meta"><i class="genericon genericon-time"></i>'.get_the_date( 'M d, Y', $object_id->ID ).'</span>' : '';
						   $thumbshow .= '</div>';
						   $thumbshow .= '</div>';
