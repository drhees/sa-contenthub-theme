...
if ( ! function_exists( 'ot_default_settings' ) ) {
  ...
  function ot_default_settings() {
    ...
    if ( ! get_option( ot_settings_id() ) ) {
      ...
      if ( mysql_num_rows( $wpdb->get_results( "SHOW TABLES LIKE '{$table_prefix}option_tree'" ) ) == 1 && $old_settings = $wpdb->get_results( "SELECT * FROM {$table_prefix}option_tree ORDER BY item_sort ASC" ) ) {
